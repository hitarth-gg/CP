...
void dfs(int node, int parent, int depth, vector<vll> &up, vll &level, vector<vll> &adj, vll &height) {
    up[node][0] = parent;
    level[node] = depth;
    height[node] = 1;
    ...
    for (int neighbor : adj[node]) {
        if (neighbor != parent) {
            dfs(neighbor, node, depth + 1, up, level, adj, height);
            height[node] = max(height[node], height[neighbor] + 1);
        }
    }
}

int jump(int u, int v, vector<vll> &up, vll &level) {
    for (int i = K - 1; i >= 0; i--) {
...
int lca(int u, int v, vector<vll> &up, vll &level) {
    if (level[u] < level[v]) swap(u, v);
    
    int originalU = u, originalV = v;
    u = jump(u, v, up, level);
    
    if (u == v) return level[originalU] - level[originalV];
    ...
    int lcaNode = up[u][0];
    return level[originalU] + level[originalV] - 2 * level[lcaNode];
}
...
void solve() {
    ...
    vll level(n + 1), height(n + 1);
    vector<vll> up(n + 1, vll(K, -1));
    
    dfs(1, -1, 1, up, level, adj, height);
    ...
    ll answer = 0;
    for (int i = 0; i < n - 1; i++) {
        answer += lca(traversal[i], traversal[i + 1], up, level);
    }
