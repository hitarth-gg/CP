...
pair<ll, ll> getNum(ll x, ll r, ll c, ll l, ll h, ll n)
{

    ll z = (l + h) / 4;
    ll half = binpow(2, n) / 2;
    ll area = half * half;
    ll l1 = l, h1 = l1 + half - 1;
    ll l2 = h1 + 1, h2 = l2 + half - 1;
    ll l3 = h2 + 1, h3 = l3 + half - 1;
    ll l4 = h3 + 1, h4 = l4 + half - 1;
    ...
    else
    {
        if (x < l + area)
        {
            return getNum(x, r, c, l, l + area - 1, n - 1);
        }
        else if (x < l + 2 * area)
        {
            return getNum(x, r + half, c + half, l + area, l + 2 * area - 1, n - 1);
        }
        else if (x < l + 3 * area)
        {
            return getNum(x, r + half, c, l + 2 * area, l + 3 * area - 1, n - 1);
        }
        else
        {
            return getNum(x, r, c + half, l + 3 * area, h, n - 1);
        }
    }
}

ll getVal(ll x, ll y, ll r, ll c, ll l, ll h, ll n)
{
    ll half = binpow(2, n) / 2;
    ll area = half * half;
    ...
    else
    {
        if (x < r + half && y < c + half)
            return getVal(x, y, r, c, l, l + area - 1, n - 1);
        else if (x >= r + half && y >= c + half)
            return getVal(x, y, r + half, c + half, l + area, l + 2 * area - 1, n - 1);
        else if (x >= r + half && y < c + half)
            return getVal(x, y, r + half, c, l + 2 * area, l + 3 * area - 1, n - 1);
        else
            return getVal(x, y, r, c + half, l + 3 * area, h, n - 1);
    }
}
...
void solve()
{
    ...
    while (q--)
    {
        ...
        if (type == "->")
        {
            ll x, y;
            cin >> x >> y;
            cout << getVal(x - 1, y - 1, 0, 0, 1, mx, n) << nl;
        }
        else if (type == "<-")
        {
            ll d;
            cin >> d;
            auto res = getNum(d, 0, 0, 1, mx, n);
            cout << res.first + 1 << " " << res.second + 1 << nl;
        }
    }
}
