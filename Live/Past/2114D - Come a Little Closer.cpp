...
void solve()
{
    ...
    vsort(x);
    vsort(y);
    ll xmin = x[0], x2 = x[1], xmax = x[n - 1], x2m = x[n - 2];
    ll ymin = y[0], y2 = y[1], ymax = y[n - 1], y2m = y[n - 2];

    // ll temp =  upper_bound(x.begin(), x.end(), xmin) - x.begin();
    ll cx1 = LB(x, xmin + 1);
    ll cx2 = x.size() - LB(x, xmax);
    ll cy1 = LB(y, ymin + 1);
    ll cy2 = y.size() - LB(y, ymax);

    ll W = xmax - xmin + 1, H = ymax - ymin + 1;
    ll ans = W * H;

    for (int i = 0; i < n; i++)
    {
        ll x = p[i].first, y = p[i].second;
        bool fx = (x == xmin && cx1 == 1) || (x == xmax && cx2 == 1);
        bool fy = (y == ymin && cy1 == 1) || (y == ymax && cy2 == 1);
        if (fx == false && fy == false)
            continue;

        ll xtmn = xmin, xtmx = xmax, ytmn = ymin, ymm = ymax;
        if (x == xmin && cx1 == 1)
            xtmn = x2;
        if (x == xmax && cx2 == 1)
            xtmx = x2m;
        ...
        if (w * h >= n)
        {
            ...
            cur = min(ceill(double(n) / h) * h, w * ceill(double(n) / w));
            ans = min(ans, cur);
        debug(ans, cur);
    }
    cout << ans << nl;
