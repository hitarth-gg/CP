...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (v[i] != -1)
        {
            dp[v[i]] = mod_add(dp[v[i]], binpow(k, m, MOD));
            for (int j = 1; j < v[i]; j++)
            {
                dp[v[i]] = mod_add(dp[v[i]], dp[j]);
            }
        }
        else
        {
            ...
            for (int z = 1; z <= 5001; z++)
                prefix[z] = mod_add(prefix[z-1], dp[z]);
            for (int j = 1; j <= k; j++)
            {
                dp2[j] = mod_add(k*dp[j], binpow(k, m, MOD));
                dp2[j] = mod_add(dp2[j], prefix[j - 1]);
            }
            dp = dp2;
            m++;
        }
