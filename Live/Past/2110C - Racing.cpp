...
void solve()
{
    ...
    vp dp(n + 1, pair<ll, ll>(0, 0));

    bool ok = 1;

    for (int i = 0; i < n; i++)
    {
        ll l = dp[i].first + max(0LL, d[i]);
        ll h = dp[i].second + (d[i] == 0 ? 0 : 1);
        if (d[i] == 1)
            l = dp[i].first + 1, h = dp[i].second + 1;
        l = max(l, ap[i].first);
        h = min(h, ap[i].second);
        if (l > h)
        {
        ...
        debug(dp[i + 1], l, h, d[i], ap[i].first, ap[i].second);
    }
