...
void rec(vll &v, int index, ll current_sum)
{
    if (index == v.size())
    {
        ans = min(ans, abs((sum - current_sum) - current_sum));
        return;
    }

    // return min(rec(v, index + 1, current_sum + v[index]), rec(v, index + 1, current_sum));
    rec(v, index + 1, current_sum + v[index]);
    rec(v, index + 1, current_sum);
}
