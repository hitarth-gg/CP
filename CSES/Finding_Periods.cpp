...
/* ---------------------- snippets ---------------------- */
// STRING: string_hashing | string_double_hashing | rabin_karp | kmp | z_function
...
void solve()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        while (curidx < n)
        {
            ll len = min(i + 1, n - curidx);
            ok &= h.substringHash(0, len - 1) == h.substringHash(curidx, curidx + len - 1);
            curidx += len;
        }
        if (ok)
        {
