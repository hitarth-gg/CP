...
void solve()
{
    ...
    {
        if (it.second >= k)
            perfectColors.push_back(occ[it.first]);
        else
            for (auto it2 : occ[it.first])
                imperfectColors.push_back(it2);
    }

    vll ans(n, 0);

    for (auto it : perfectColors)
    {
        ll shine = k;
        for (auto it2 : it)
            ans[it2] = max(shine--, 0LL);
    }

    ll shine = 0;
    ll tz = imperfectColors.size() / k;
    for (ll i = 0; i < k * tz; i++)
        ans[imperfectColors[i]] = shine++ % k + 1;
    printVec(ans);
}
