...
void solve()
{
    ...
    // gcd * lcm = a*b
    // ll gcd_ = gcd(s1.size(), s2.size());
    // ll lcm_ = s1.size()*s2.size()/gcd_;
    ll lcmA = lcm(s1.size(), s2.size());
    trace3(s1.size(), s2.size(), lcmA);

    if(mul(s1, lcmA/s1.size()) == mul(s2, lcmA/s2.size()))
        cout << mul(s1,lcmA/s1.size()) << nl;
    else
        cout << -1 << nl;
