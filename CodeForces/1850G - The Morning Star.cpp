...
// PROBLEM WHERE UNORDERED MAP FAILS BUT UNORDERED MAP WITH CUSTOM HASH WORKS
...
void solve()
{
    ...
    unordered_map<ll, ll, custom_hash> mp3;
    unordered_map<ll, ll, custom_hash> mp4;
    ll ans = 0;
    for(int i = 0; i<n; i++)
    {
        int key1 = v[i].second - v[i].first;
        int key2 = v[i].second + v[i].first;
        int key3 = v[i].first;
        int key4 = v[i].second;

        if(mp1.contains(key1))
            ans += 2*mp1[key1];
        if(mp2.contains(key2))
            ans += 2*mp2[key2];
        if(mp3.contains(key3))
            ans += 2*mp3[key3];
        if(mp4.contains(key4))
            ans += 2*mp4[key4];
        mp1[key1]++;
        mp2[key2]++;
        mp3[key3]++;
