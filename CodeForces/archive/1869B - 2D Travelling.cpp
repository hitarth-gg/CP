...
{
    re(n, k, a, b);
    ll majorDist1 = LONG_LONG_MAX;
    ll majorDist2 = LONG_LONG_MAX;
    vp v;
    for (int i = 0; i < n; i++)
    {
        re(t1, t2);
    ...
    for (int i = 0; i < k; i++)
    {
        majorDist1 = min(majorDist1, abs(v[a - 1].first - v[i].first) + abs(v[a - 1].second - v[i].second));
        majorDist2 = min(majorDist2, abs(v[b - 1].first - v[i].first) + abs(v[b - 1].second - v[i].second));
    }
    ll sum1 = majorDist1 + majorDist2;
    ll sum2 = abs(v[a - 1].first - v[b - 1].first) + abs(v[a - 1].second - v[b - 1].second);
    if(k>=2)
