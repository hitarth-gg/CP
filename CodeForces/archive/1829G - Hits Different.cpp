...
{
    l--;
    ll sum = r * (r + 1) * (2 * r + 1) / 6 - l * (l + 1) * (2 * l + 1) / 6;
    return sum;
}
...
void solve()
{
    ...
    {
        ll min_num = mn[level];
        ll max_num = min_num + level - 1;
        
        left = max(left - level - 1, min_num);
        right = min(right - level, max_num);

        sum += sum_n2(left, right);
