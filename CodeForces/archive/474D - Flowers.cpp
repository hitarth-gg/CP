...
ll combis(ll n, ll k)
{
    ...
    else
    {
        ...
        if (n < 0)
            return 0;
        dp[n] = (combis(n - k, k) + combis(n - 1, k)) % MOD; // remove whites (means that you added white to the string) + remove reds (means that you added red to the string)
        return dp[n];
    }
}
...
void genDP(ll k)
{
    for (int i = 1; i < size_dp; i++)
        // p[i] = mod_add(p[i-1], combis(i, k), MOD);
        p[i] = p[i - 1] + combis(i, k);
}
...
void solve()
{
    ...
    for (int i = 0; i < t; i++)
    {
        re(t1, t2);
        // ll ans = p[t2] - p[t1 - 1];
        ll ans = mod_sub(p[t2], p[t1 - 1], MOD);
        cout << ans << nl;
    }
}
// clang-format off
