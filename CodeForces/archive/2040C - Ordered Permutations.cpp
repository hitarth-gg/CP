...
void f(ll num, ll l, ll r, ll k, vll &ans)
{
    if (l == r)
    {
        ans[l] = num;
    ...
    ll empty_cells = r - l + 1;
    ll rem_cells = empty_cells - 1;
    ll sz = rem_cells - 1; // as 2^(n-1) karna hai

    if (sz < 42 && k > (1LL << sz)) // k can be max 1e12
    {
        ans[r] = num;
        f(num + 1, l, r - 1, k - (1LL << sz), ans);
    }
    else
    {
        ans[l] = num;
