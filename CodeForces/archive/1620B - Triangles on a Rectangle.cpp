...
tuple<ll, ll, ll> sussybaka(ll x)
{
    ...
    tuple<ll, ll, ll> x1 = make_tuple(min, max, x);
    return x1;
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        tuple<ll, ll, ll> x1 = sussybaka(h);
        tuple<ll, ll, ll> x2 = sussybaka(h);
        tuple<ll, ll, ll> y1 = sussybaka(w);
        tuple<ll, ll, ll> y2 = sussybaka(w);

        ll areaX2 = 0;

        vector<tuple<ll,ll,ll>> v = {x1,x2,y1,y2};

        for(auto it : v)
        {
            ll temp = abs(get<0>(it)-get<1>(it))*get<2>(it);
            if(temp > areaX2)
                areaX2 = temp;
        }
        cout << areaX2 << nl;
