...
vector<bool> isPrime = sieve(10000002);
...
vll get_factors(ll num, ll upper_limit = 10000002, bool reset = false);
...
void solve()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        vll fs = get_factors(v[i]);
        ll po = v_count(fs, fs[0]);
        if (fs.size() == po)
        {
            a.pb(-1);
            b.pb(-1);
        ...
        else
        {
            ...
            // ll d1 = binpow(fs[0], po);
            // ll d2 = v[i] / d1;
            ll d2 = 1;
            for (auto it = fs.begin() + po; it != fs.end(); ++it)
                d2 *= *it;
            a.pb(fs[0]);
            b.pb(d2);
        }
