...
public:
    DisjointSet(ll n)
    {
        size.resize(n + 1, 0); // n+1 so that it works for both 0 & 1 based indexing
        parent.resize(n + 1);
        for (int i = 0; i <= n; i++)
            parent[i] = i;
    }
...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        while (true)
        {
            ...
            if (vis[curr] && curr != start)
            {
                vis[curr] = 1;
                break;
            }
            if (vis[curr] && curr == start && cnt > 2)
            {
                vis[curr] = 1;
                cycle++;
                break;
    ...
    ll open = components - cycle;

    ll mn = cycle + (open > 0 ? 1 : 0);
    ll mx = cycle + open;
    cout << mn << " " << mx << nl;
}
