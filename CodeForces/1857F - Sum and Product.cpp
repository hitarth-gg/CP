...
void solve()
{
    ...
    while (q--)
    {
        ...
        if (mp2.contains({x, y}))
        {
            ans.push_back(mp2[{x, y}]);
            continue;
        }

        ll sum = 0;
        pair<ll, ll> roots_aj = root(x, y);
        debug(roots_aj);
        if (roots_aj.first != -INF)
        {
            ll aj = roots_aj.first;
            ll ai = y / aj;

            if (mp.contains(ai) && mp.contains(aj))
            {
                for (auto it : mp[ai])
                {
                    ll t = mp[aj].size() - UB(mp[aj], it);
                    if (t == 0)
                        break;
                    sum += t;
                }
        ...
        if (roots_aj.second != -INF && roots_aj.second != roots_aj.first)
        {
            ll aj = roots_aj.second;
            ll ai = y / aj;
