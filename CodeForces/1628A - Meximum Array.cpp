...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (!st1.contains(v[i]) && v[i] < next_mex)
            next_mex = v[i]; // rollback mex calculation of remaining array i.e. the array from i+1 to n-1

        st2.insert(v[i]);

        while (st2.contains(cur_mex))
            cur_mex++;
        if (cur_mex == mex)
        {
            debug(mex);
