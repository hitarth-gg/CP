...
// # dream7's solution: https://codeforces.com/contest/1739/submission/173990121

void solve()
{
    // hint : generate cases for n=4
    re(n);
    ll turn = 0;
    ll ans = 0;
    ll alex = 0;
    ...
    {
        if (turn == 0)
            alex = mod_add(alex, nCrModPFermat(i - 1, i / 2));
        else
            alex = mod_add(alex, nCrModPFermat(i - 2, (i - 2) / 2 - 1));
        turn = !turn;
    }
    ll bob = mod_sub(nCrModPFermat(n, n / 2) - 1, alex);
    cout << alex << " " << bob << " " << 1 << nl;
}
