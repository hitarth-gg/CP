...
void solve()
{
    ...
    // We are basically finding the first index of the last element, tspmo
    auto eval = [&](ll mid)
    {
        vll p(n + 1, 0);
        for (int i = 0; i <= mid; i++)
        ...
        for (int i = 0; i < m; i++) {
            ll ix1 = segments[i].first;
            ll ix2 = segments[i].second;

            ll ones = p[ix2] - p[ix1 - 1];
            ll zeroes = ix2 - ix1 + 1 - ones;
            if (ones > zeroes)
                return true;
        }
        return false;
