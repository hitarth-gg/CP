...
void solve() 
{
    ...
    // lowkey a retarded question, tspmo

    /*
        we'll work from final array to initial array i.e. from [2 3 1] to [1 2 3]
        then we'll reverse the operations along with the pairs to get the final output
    */
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        /* keep moving the current element (x) to the right */
        for (int j = ix + 1; j < n; j++)
            ans.pb({v[j], x});

        /*
            then once it reaches the end, move the current element to the left, 
            back to its previous position (ix)
        */
        for (int j = n - 1; j > ix; j--)
            ans.pb({x, v[j]}); //

        /*
            then move the current element to the left until it reaches its original position
            (which is the index x-1 (as zero based indexing is there)),
            remember that we are building the array [1, 2, 3, 4, 5, ..., n].
            `cnt` is the number of elements that are already in their final position.
            Concurrently increment the indexes of the elements that are being swapped to the right of vaule x
        */
        for (int j = ix - 1; j >= cnt; j--)
        {
            ans.pb({x, v[j]});
            v[j + 1] = v[j];
            index[v[j]]++;
        }
        v[cnt++] = x; // move the current element to its original position and increment the count
    }
    cout << ans.size() << nl;
    reverse(ans.begin(), ans.end()); // reversing the operations
    for (auto it : ans)
        cout << it.second << " " << it.first << nl; // reversing the pairs
}
