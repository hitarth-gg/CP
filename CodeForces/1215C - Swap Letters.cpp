...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        a1 += (s1[i] == 'a'), b1 += (s1[i] == 'b'), a2 += (s2[i] == 'a'), b2 += (s2[i] == 'b');
    ...
    if ((a1 + a2) % 2 == 0 && (b1 + b2) % 2 == 0)
    {
        ...
        ans += m["ba"].size() / 2;
        ans += m["ab"].size() / 2;
        if (m["ab"].size() % 2 == 1 && m["ba"].size() % 2 == 1)
            ans += 2;
        cout << ans << nl;

        for (int i = 0; i + 1 < m["ab"].size(); i += 2)
            cout << m["ab"][i] + 1 << " " << m["ab"][i + 1] + 1 << nl;
        for (int i = 0; i + 1 < m["ba"].size(); i += 2)
            cout << m["ba"][i] + 1 << " " << m["ba"][i + 1] + 1 << nl;

        if (m["ab"].size() % 2 == 1 && m["ba"].size() % 2 == 1)
        {
            cout << m["ab"].back() + 1 << " " << m["ab"].back() + 1 << nl;
            cout << m["ab"].back() + 1 << " " << m["ba"].back() + 1 << nl;
        }
    }
    else
    {
