...
/* ---------------------- RECURSIVE --------------------- */
ll MAXN = 1<<21;
matrix<ll> dp(21, MAXN, -1);
ll f(ll boy, ll girlsHash, matrix<ll> &v)
{
    ll n = v[0].size();
    ...
    if(dp[boy][girlsHash] != -1)
        return dp[boy][girlsHash];

    ll ans = 0;

    for(ll girl = 0; girl<n; girl++)
    {
        if((girlsHash & (1 << girl)) > 0)
            continue;
        else if(v[boy][girl] == 1)
        {
            ll t = f(boy+1, (girlsHash | (1<<girl)), v);
            ans = mod_add(ans, t);
        }
    }
    return dp[boy][girlsHash] = ans;    
}
...
void solve()
{
    ...
    for (ll i = 0; i < MAXN; i++)
    {
        ...
        for (ll girl = 0; girl < n; girl++)
        {
            if ((i & (1 << girl)) == 0 && v[boy][girl] == 1)
                dp[i | (1 << girl)] = mod_add(dp[i | (1 << girl)], dp[i]);
        }
    }
    cout << dp[(1 << n) - 1] << nl;
}
