...
{
    re(n, w);
    // since value of w can be upto 1e9, we can't use dp[w] as it will be too large
    // so we will use a different approach, notice that total sum of values can be atmost 1e5
    // so we can use dp[1e5] to store the minimum weight required to get that value
    // then we can iterate from 1 to 1e5 and find the maximum value that can be achieved with weight <= w
    ll mx = 1e5 + 10;
