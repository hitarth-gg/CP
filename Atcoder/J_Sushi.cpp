...
lld f(ll x, ll y, ll z)
{
    ...
    if (dp[x][y][z] > 0)
        return dp[x][y][z];
    // lld ret = 1 + (1. * x * f(x - 1, y, z) + 1. * y * (f(x + 1, y - 1, z)) + 1. * z * (f(x, y + 1, z - 1))) / n;
    lld ret = 1;

    if (x > 0)
        ret += (1. * x * f(x - 1, y, z)) / n;
    if (y > 0)
        ret += (1. * y * f(x + 1, y - 1, z)) / n;
    if (z > 0)
        ret += (1. * z * f(x, y + 1, z - 1)) / n;

    lld p0 = 1. * (n - x - y - z) / n;
    ret /= 1. - p0;
    return dp[x][y][z] = ret;
}
// https://www.youtube.com/watch?v=M_r02oqyKgY&list=PL6h2Gn3JK5Lnew3sVMFp0sNWBOgsDo62x&index=18&ab_channel=NachiketKanore
void solve()
{
    cin >> n;
    reV(v, n);
